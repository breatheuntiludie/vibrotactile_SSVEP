...
OX1 = zeros(Length_epoch, 1);
for local = 1:500
    OX1(local, 1) = (local-1)*2;
end;
...
Fpz = zeros(time_fpz*freq_discr, 1);
Cz = zeros(time_fpz*freq_discr, 1);
Stim = zeros(time_fpz*freq_discr, 1);
OX = zeros(time_fpz*freq_discr, 1);
check = zeros(50, 1);
p1 = 1;
f = 1;
while p1 < time_fpz*freq_discr
    Fpz(p1,1) = Data(Ip + p1, 2);
    Cz(p1, 1) = Data(Ip + p1, 6);
    if (Fpz(p1, 1) < -80)
        if (check(f, 1) - p1 < -10 && check(f, 1) ~= 0)
            f = f + 1;
        end;
        check(f, 1) = p1;
    end;
...
for j = 1:N
    ...
    for p2 = 1:freq_discr/freq
        Stim(p2 + (j - 1)*freq_discr/freq, 1) = res1;
    end;
end;
for loc = 1:time_fpz*freq_discr
    OX(loc, 1) = loc;
end;
plot(OX,Fpz,OX, Stim, OX, Cz);
...
while (c < 50 && c ~= 0)
    ...
    if (point2 ~= 0)
        r = 0;
        while (Stim(point2,1) == -50)
            point2 = point2 - 1;
        end;
        if (point2 > 0)
            for i = 1:Length_epoch
                SSep(i, 9) = SSep(i, 9) - Data(Ip + point2 + i, ch);
            end;
...
SSep1 = SSep / Norminette(9,1);
figure;
plot(OX1,SSep1);
xlabel('Time, ms');
ylabel('Amplitude, uV');
title(['Averaged P300: ', num2str(Norminette(9,1))]);
...
%     for i = 1:Length_epoch
%         if Signals(Is,1) == 8
%             SSep(i, 9) = SSep(i, 9) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 0
%             SSep(i, 1) = SSep(i, 1) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif (Signals(Is,1) == 1 || Signals(Is,1) == 10)
%             SSep(i, 2) = SSep(i, 2) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 2
%             SSep(i, 3) = SSep(i, 3) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 3
%             SSep(i, 4) = SSep(i, 4) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 4
%             SSep(i, 5) = SSep(i, 5) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 5
%             SSep(i, 6) = SSep(i, 6) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 6
%             SSep(i, 7) = SSep(i, 7) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         elseif Signals(Is,1) == 7
%             SSep(i, 8) = SSep(i, 8) - Data(Ip + i + (j - 1)*(freq_discr/freq), ch);
%         end;
%     end;
%     [b, a] = sos2tf(SOS,G);
%     SSep = filter(b,a, SSep);
